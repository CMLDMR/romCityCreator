...
QList<QPointF> ConcaveHull::concavehull(const QList<QPointF> &coords, double chi_factor)
{
    std::vector<double> _coords;

    for( const auto &point : coords ){
        _coords.push_back(point.x());
        _coords.push_back(point.y());
    }
    ...
    Delaunator::Delaunator d(_coords);
    ...
    for( int i = 0 ; i < d.get_hull_coords().size()-1 ; i += 2 ){
        pList.push_back(QPointF{d.get_hull_coords()[i],d.get_hull_coords()[i+1]});
    }
